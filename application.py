...
# Connect to item catalog SQL DB
...
# ----------------------------------------------------
# JSON Endpoints Required to show detailed information
# ----------------------------------------------------
...
# Item by Item Endpoint requested in first project review 14/11/2018
...
def editCategory(category_id):
    ...
    if editedCategory.user_id != login_session['user_id']:
        return "<script>function myFunction() {alert('You are not auth" \
               "orized!')}</script><body onload='myFunction()'>"
    if request.method == 'POST':
...
def deleteCategory(category_id):
    ...
    if categoryToDelete.user_id != login_session['user_id']:
        return "<script>function myFunction() {alert('You are not autho" \
               "rized!')}</script><body onload='myFunction()'>"
    if request.method == 'POST':
...
def editCatalogItem(category_id, catalog_item_id):
    ...
    if editedItem.user_id != login_session['user_id']:
        return "<script>function myFunction() {alert('You are not aut" \
               "horized!')}</script><body onload='myFunction()'>"
    if request.method == 'POST':
        if request.form['name']:
...
def fbconnect():
    ...
    url = 'https://graph.facebook.com/oauth/access_token?grant_type=fb_ex' \
          'change_token&client_id=%s&client_secret=%s&fb_exchange_t' \
          'oken=%s' % (app_id, app_secret, access_token)
    h = httplib2.Http()
    ...
    url = 'https://graph.facebook.com/v2.8/me?fields=id%2Cname%2Ce' \
          'mail%2Cpicture&access_token=' + access_token
    h = httplib2.Http()
    result = h.request(url, 'GET')[1]
    ...
    # Store token in the login_session
...
def fbdisconnect():
    ...
    url = 'https://graph.facebook.com/%s/permissions?access_' \
          'token=%s' % (facebook_id, access_token)
    h = httplib2.Http()
...
def gconnect():
    ...
    try:
        # Change authorization code into a credentials object
    ...
    # Cancel if token threw an error.
    if result.get('error') is not None:
        response = make_response(json.dumps(result.get('error')), 500)
        response.headers['Content-Type'] = 'application/json'
        return response

    # Verify user - access token dependency.
    ...
    # Verify access token - app dependency.
...
# User login additional functions
def getUserID(email):
    try:
...
# Disconnect general functions (FB or Google)
...
if __name__ == '__main__':
    app.secret_key = '-KteQMfnAAws7-8kPVu8aFwl'
    app.debug = True
